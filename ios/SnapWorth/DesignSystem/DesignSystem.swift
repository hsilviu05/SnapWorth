...
/// Tag: the app icon's price tag, come to life. The artwork is in
/// Assets.xcassets/Mascot, generated by marketing/mascot/export_app_assets.py.
///
/// Decorative wherever it appears, so it is hidden from VoiceOver and the
/// text beside it carries the meaning. Each image's Dark appearance is the
/// same drawing with a cream edge: the espresso outline disappears on the
/// dark ground and on the camera's charcoal scrim.
struct TagMascot: View {
    ...
    static let blinkAssetName = "TagBlink"
    /// Width over height of the artwork (137.5 × 160 pt at design size).
    static let aspectRatio: CGFloat = 137.5 / 160
    ...
    var body: some View {
        Group {
            if reduceMotion {
                figure(asset: mood.assetName, lift: 0, squash: 0)
            } else {
                // 30 fps is plenty for a 0.8 Hz bob, at half the work of 60.
                TimelineView(.animation(minimumInterval: 1.0 / 30)) { context in
                    let t = context.date.timeIntervalSinceReferenceDate
                    let wave = CGFloat(sin(t * 2 * .pi * 0.8))
                    // Only the resting face blinks: the blink artwork carries
                    // the resting smile, and the other moods have their own mouths.
                    let blinking = mood == .happy && t.truncatingRemainder(dividingBy: 3.6) < 0.12
                    figure(asset: blinking ? Self.blinkAssetName : mood.assetName,
                           lift: max(0, wave) * height * 0.035,
                           squash: -wave * 0.02)
                }
            }
        }
    ...
    private func figure(asset: String, lift: CGFloat, squash: CGFloat) -> some View {
        ZStack(alignment: .bottom) {
            // Ground shadow: stays on the floor and shrinks as Tag rises.
            // Light only; on the dark ground it would not show.
            if colorScheme == .light {
                Ellipse()
                    .fill(Color.snapEspresso.opacity(0.12))
                    .frame(width: height * 0.6 * (1 - 2 * lift / height), height: height * 0.07)
            }
            Image(asset)
                .resizable()
                .scaledToFit()
                ...
                .offset(y: -lift - height * 0.02)
        }
        // Room for the highest point of the bob, so Tag never draws over the view above.
        .frame(width: height * Self.aspectRatio, height: height * 1.06, alignment: .bottom)
    }
}

// ═══════════════════════════════════════════════════════════════════
// MARK: - Analyzing Overlay  (Tag + rotating copy)
// ═══════════════════════════════════════════════════════════════════
...
struct AnalyzingOverlay: View {
    ...
    /// Tag's face for each message above, in the same order.
    private let moods: [TagMascot.Mood] = [.happy, .wow, .happy, .joy]
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 20) {
                // Always the Dark artwork: the scrim is charcoal in both themes.
                TagMascot(mood: moods[messageIndex], height: 96)
