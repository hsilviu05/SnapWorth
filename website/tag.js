...
function Tag(btn) {
  ...
  var bubble = btn.parentNode.querySelector(".tag-bubble"),
    ...
    hopAt = -1e9, landAt = -1e9, blinkAt = now() + 2500, bEnd = 0, glanceAt = 0;
  ...
  function words(text) { // the label on up to two lines, each fitted
  ...
  function say(text) {
    ...
    clearTimeout(say.t); say.t = setTimeout(function () { bubble.classList.remove("is-on"); }, 2400);
  }
  function flipTo(target, done) {
    goal = target; onGoal = done;
    if (!raf) { // reduced or paused: jump
      phi = goal; onGoal = null;
      var b = M.cos(phi) < 0; cls.toggle("is-back", b); svg.style.setProperty("--sx", b ? -1 : 1);
      if (done) done();
    }
  }
  // wow, hop, flip, hold, flip back (joy), land; a new find resumes the flip
  function react(text, what) {
    ...
    var still = raf && !phi; // phi is exactly 0 at rest
    if (still) hopAt = now();
    later(function () {
      flipTo(PI, function () {
        later(function () {
  ...
  function frame(t) {
    ...
    if (!led && !hero && t > glanceAt) { // 404 glances until a pointer leads
    ...
    svg.style.cssText = "--hop:" + hop + "px;--sx:" + sx * c + ";--sy:" + sy + ";--lean:" + lean +
      "deg;--lx:" + ex + "px;--ly:" + ey + "px;--al:" + al + "deg;--ar:" + ar + "deg;--tw:" +
      (hover ? 1 + 0.18 * M.abs(M.sin(s * 5)) : 1) + ";--sh:" + (1 + hop / 240);
  }
  function run() {
  ...
  new IntersectionObserver(function (e) { seen = e[0].isIntersecting; run(); }).observe(btn);
  ...
  btn.addEventListener("click", function () {
    ...
    each(chips, function (ch, j) { if (attr(ch, "aria-pressed") === "true") i = j; });
